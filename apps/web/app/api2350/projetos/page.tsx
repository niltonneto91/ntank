...
import { listarProjetosAPI2350, excluirProjetoAPI2350, salvarProjetoAPI2350 } from "@/lib/db";
import { classificarCategoriaOPS } from "@ntank/calc-core";
...
const LABEL_CLASSE_NFPA: Record<string, string> = {
  "I":             "Cl. I",
  "II":            "Cl. II",
  "IIIA":          "Cl. IIIA",
  "IIIB":          "Cl. IIIB",
...
/** Calcula a categoria OPS de forma segura — retorna null se falhar. */
function calcularCategoria(p: ProjetoAPI2350): number | null {
  try {
    return classificarCategoriaOPS({
...
function ProjetoCard({
  ...
  const categoria = useMemo(() => calcularCategoria(p), [p]);
  ...
  return (
    <Card className="flex flex-col gap-3">
      <div className="flex items-start justify-between gap-3">
        ...
        <Badge cor="verde">
          {LABEL_CLASSE_NFPA[p.produto.classeNFPA] ?? p.produto.classeNFPA}
        </Badge>
      </div>
      ...
      <dl className="grid grid-cols-4 gap-2 text-sm">
        ...
        <div>
          <dt className="text-carbono-500">H total</dt>
          <dd className="font-semibold tabular">{m(p.geometria.H_total_m)}</dd>
        </div>
        <div>
      ...
      {/* Badge de categoria OPS */}
      {categoria !== null && (
        <div className="flex items-center gap-2">
          <span className={`rounded px-2 py-0.5 text-xs font-bold ${COR_CATEGORIA[String(categoria)] ?? ""}`}>
            Categoria {categoria}
          </span>
          <span className="text-xs text-carbono-400">OPS</span>
        </div>
      )}

      <div className="flex items-center justify-between gap-2">
        <Link href={`/api2350/${p.id}`}>
...
function agruparPorPasta(
  projetos: ProjetoAPI2350[],
): Array<{ pasta: string; lista: ProjetoAPI2350[] }> {
  const map = new Map<string, ProjetoAPI2350[]>();
...
export default function ProjetosAPI2350Page() {
  const [projetos, setProjetos] = useState<ProjetoAPI2350[] | null>(null);
  ...
  async function handleExcluir(id: string) {
    ...
    await excluirProjetoAPI2350(id);
  ...
  async function handleMover(id: string, pasta: string) {
    ...
    if (!proj) return;
    // ProjetoAPI2350.pasta é string (não undefined) — usa "" como "sem pasta"
    const atualizado: ProjetoAPI2350 = { ...proj, pasta: pasta };
    await salvarProjetoAPI2350(atualizado);
    setProjetos((prev) =>
  ...
  const gruposComExtras = useMemo(() => {
    ...
    const extras = pastasExtras
      ...
      .map((nome) => ({ pasta: nome, lista: [] as ProjetoAPI2350[] }));
      ...
      <section className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-carbono-600">
            ...
            <span className="font-semibold">Overfill Prevention</span>.
          </p>
        </div>
        <div className="flex gap-2">
      ...
      {projetos && projetos.length === 0 && pastasExtras.length === 0 && (
        <Card title="Comece pelo primeiro cálculo">
          <p className="text-sm text-carbono-600">
            Você ainda não tem projetos de prevenção de transbordamento salvos. Crie um
            novo para classificar a categoria OPS, calcular o tempo e volume de resposta
            e verificar os níveis MW / HH / AOPS / CH.
          </p>
          <div className="mt-4">
            <Link href="/api2350/novo">
